...
def solve_QB(rewards, lambda_i, lambda_j, mu_i, save_lp=True):
    ...
    # Constraint (14): Flow conservation for active types
    for i in I:
        model.addConstr(
    ...
    # Constraint (15): Abandonment rate constraint
    for i in I:
        for j in J:
            model.addConstr(
                mu_i[i] * x[i, j] <= (x_j[j] + sum(x[k, j] for k in I)) * x_a[i],
                name=f"Abandonment_{i}_{j}"
            )
    ...
    if model.status == GRB.OPTIMAL:
        ...
        # Extract passive unmatched buffer values
        for j in J:
